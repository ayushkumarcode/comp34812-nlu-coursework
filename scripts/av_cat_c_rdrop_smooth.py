...
def main():
    ...
    print_metrics(metrics, "AV Cat C R-Drop+Smooth")

    pp = pred_dir = PROJECT_ROOT / 'predictions'
    save_predictions(
        final_preds,
        pp / 'av_Group_34_C_rdrop_smooth_v2.csv'
    )
    np.save(
        pp / 'av_cat_c_rdrop_smooth_v2_probs.npy',
        all_probs
    )
